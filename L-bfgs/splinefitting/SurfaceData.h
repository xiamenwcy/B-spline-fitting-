...
//	Polyhedron *get_domain_mesh();
	...
	static bool interection_st( Triangle_2 tri,Segment_2 seg);//三角形与边相交
	static bool  intersection_rt(Triangle_2 tri,Iso_rectangle_2 rec,double *Area);//求三角形与矩形的交集

	void  update_curvature_color();
	static void  update_knots(double *uknots,double *vknots,int unum,int vnum,MyMesh polymesh,Mesh *m_pOriginalMesh,double curaverage,double *func, double *grad);
