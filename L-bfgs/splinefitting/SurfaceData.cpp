...
bool   CSurfaceData::knots_iteration()
{
	...
	//求解平均曲率积分总和
     double sum=0.0;
	  ...
	  curaverage=sum/((unum-1)*(vnum-1));
//L-BFGS方法
	  real_1d_array x;
	  double *x0=new double[unum+vnum-4];
	  for (int i=0;i<unum-2;i++)
	  {
		  x0[i]=uknots[i+1];
	  }
	  for (int i=unum-2;i<unum+vnum-4;i++)
	  {
		  x0[i]=vknots[i-unum+3];
	  }
	  x.setcontent(unum+vnum-4,x0);
	  delete []x0;
	  double epsg = 1e-10;
	  double epsf = 0;
	  double epsx = 0;
	...
	result<<"迭代结束的原因:"<<endl;
	cout<<"迭代结束的原因:"<<endl;
	string report[]={"internal integrity control detected  infinite  or  NAN  values  in function/gradient. Abnormal termination signalled.",
		"gradient verification failed.","relative function improvement is no more than EpsF.","relative step is no more than EpsX.",
		" gradient norm is no more than EpsG","MaxIts steps was taken",
		"stopping conditions are too stringent,further improvement is impossible,X contains best point found so far.",
		" terminated    by  user  who  called  minlbfgsrequesttermination().X contains point which was current accepted when  termination request was submitted."};
   switch (int(rep.terminationtype))
   {
   case  -8:
		   cout<<report[0]<<endl;
		   result<<report[0]<<endl;
		   break;
	  
   case  -7:
		   cout<<report[1]<<endl;
           result<<report[1]<<endl;
		   break;
	   
   case  1:
	   
		   cout<<report[2]<<endl;
		    result<<report[2]<<endl;
		   break;
	   
   case  2:
	   
		   cout<<report[3]<<endl;
		    result<<report[3]<<endl;
		   break;
	  
   case 4:
	  
		   cout<<report[4]<<endl;
		    result<<report[4]<<endl;
		   break;
	  
   case  5:
	      cout<<report[5]<<endl;
		   result<<report[5]<<endl;
		   break;
	  
   case  7:
	   
		   cout<<report[6]<<endl;
		    result<<report[6]<<endl;
		   break;
	  
   case  8:
	  
		   cout<<report[7]<<endl;
		    result<<report[7]<<endl;
		   break;
	  
   default:
	   cout<<"iteration  failure.";
	   result<<"iteration  failure.";
...
void   CSurfaceData::function1_grad(const real_1d_array &x, double &func, real_1d_array &grad, void *ptr) 
{
	//
	// this callback calculates f(x)
	// and its derivatives 
	
//这里x实际上分两部分，一部分ukonts[1],.....uknots[unum-2];另一部分vkonts[1],....vknots[vnum-2]
//因此x共有unum+vnum-4个变量;
	int unum,vnum;
    CSurfaceData *mydata=(CSurfaceData*)surfacedata_;
	unum=mydata->unum;
	vnum=mydata->vnum;
	MyMesh polymesh=mydata->polymesh;
	Mesh *m_pOriginalMesh=mydata->m_pOriginalMesh;
	double curaverage=mydata->curaverage;
    double *uknots=new double[unum];
	double *vknots=new double[vnum];
	uknots[0]=0;
	vknots[0]=0;
	uknots[unum-1]=1.0;
	vknots[vnum-1]=1.0;
   ...
   func=0;
   double *grad2=new double[unum+vnum-4];
   memset(grad2,0,(unum+vnum-4)*sizeof(grad2));   
  update_knots(uknots,vknots,unum,vnum,polymesh,m_pOriginalMesh,curaverage,&func,grad2);
  for (int i=0;i<unum+vnum-4;i++)
  {
	  grad[i]=grad2[i];
  }
  delete[] uknots;
	...
	for(int j=0;j<vnum;j++)
		...
		for(int j=0;j<vnum-1;j++)
			...
			for (f_it=m_pOriginalMesh->faces_begin();f_it!=f_end;++f_it)
			{
				...
				++fv_it;
				C=m_pOriginalMesh->texcoord2D(*fv_it);
				double xmin,xmax,ymin,ymax;int xmin_index=0,xmax_index=0,ymin_index=0,ymax_index=0;
				 Triangle_2  tri2(Point_2 (A[0],A[1]),Point_2 (B[0],B[1]),Point_2 (C[0],C[1]));
				xmax=(A[0]>B[0]?(A[0]>C[0]?A[0]:C[0]):(B[0]>C[0]?B[0]:C[0]));
				xmin=(A[0]<B[0]?(A[0]<C[0]?A[0]:C[0]):(B[0]<C[0]?B[0]:C[0]));
				ymax=(A[1]>B[1]?(A[1]>C[1]?A[1]:C[1]):(B[1]>C[1]?B[1]:C[1]));
				ymin=(A[1]<B[1]?(A[1]<C[1]?A[1]:C[1]):(B[1]<C[1]?B[1]:C[1]));
				...
				//找到三角形所在的矩形区域范围:vknots[ymin_index]<=ymin<vknots[ymin_index+1];vknots[ymax_index]<ymax<=vknots[ymax_index+1]
				...
				double t22=m_pOriginalMesh->property(f_mean_curvature2,*f_it);
				for (int i=xmin_index;i<=xmax_index;i++)
					for (int j=ymin_index;j<=ymax_index;j++)
					{
						int  index=j*(unum-1)+i;
						f2_it=polymesh.faces_begin();
						advance(f2_it,index);//找到矩形所属的面
						Iso_rectangle_2 rec(uknots[i],vknots[j],uknots[i+1],vknots[j+1]);
						double Area=-1.0;
						if(intersection_rt(tri2,rec,&Area))
						{
			...
			{
				g=abs(polymesh.property(f_area,*face_iter)-curaverage)/curaverage;
...
bool CSurfaceData::interection_st( Triangle_2 tri,Segment_2 seg)
{
	CGAL::Object result;
	Point_2 ipoint;
	Segment_2 iseg;
...
bool  CSurfaceData::intersection_rt(Triangle_2 tri,Iso_rectangle_2 rec,double *Area)
{
	...
	if (assign(itri, result)) 
	{
		*Area=itri.area();
		return true;
	} 
	else if (assign(vecpoint, result))
	{
		Polygon_2 P(vecpoint.begin(),vecpoint.end());
		*Area=P.area();
		return  true;
	}
	else if (assign(ipoint, result)||assign(iseg,result))
	{
		*Area=0;
		return  true;
	}
...
void  CSurfaceData::update_knots(double *uknots,double *vknots,int unum,int vnum,MyMesh polymesh,Mesh *m_pOriginalMesh,double curaverage,double *func, double *grad)
{
	 //update_polymesh
	polymesh.clear();
	// generate vertices
	...
	OpenMesh::EPropHandleT<double> edge_curvature;  //增加与每条边相交的三角形的曲率属性
	polymesh.add_property(edge_curvature,"edge_curvature");

	OpenMesh::EPropHandleT<int> num_curvature;  //增加与每条边相交的三角形的个数属性
	polymesh.add_property(num_curvature,"num_curvature");
   
	MyMesh::EdgeIter  ed;
	for (ed=polymesh.edges_begin();ed!=polymesh.edges_end();++ed)
	{
		polymesh.property(edge_curvature,*ed)=0.0;
		polymesh.property(num_curvature,*ed)=0;
	}
	...
	for (f_it=m_pOriginalMesh->faces_begin();f_it!=f_end;++f_it)
	{
		  ...
		  ++fv_it;
		  C=m_pOriginalMesh->texcoord2D(*fv_it);
		 double xmin,xmax,ymin,ymax;int xmin_index=0,xmax_index=0;int ymin_index=0,ymax_index=0;
		 //找到三角形所在的矩形区域范围
 ...
 //找到三角形所在的矩形区域范围:vknots[ymin_index]<=ymin<vknots[ymin_index+1];vknots[ymax_index]<ymax<=vknots[ymax_index+1]
       ...
       for (int i=xmin_index;i<=xmax_index;i++)
			...
			//先遍历各列的边
		{
			...
			for(i=xmin_index;i<=xmax_index+1;i++)
			{
				if (i==1)
				{
					if (ymin_index==0)
					{
						...
						MyMesh::HalfedgeHandle  he=polymesh.halfedge_handle(fe,0);
						MyMesh::VertexHandle     fv=polymesh.from_vertex_handle(he),tv=polymesh.to_vertex_handle(he);
						MyMesh::Point     p1=polymesh.point(fv),p2=polymesh.point(tv);
						bool is=interection_st(tri,Segment_2(Point_2(p1[0],p1[1]),Point_2(p2[0],p2[1])));
						if (is)
						{
							polymesh.property(edge_curvature,fe)+=t22;
							polymesh.property(num_curvature,fe)++;
						}
						for (j=2;j<=ymax_index+1;j++)
						{
							r=3*m1+1+(2*m1+1)*(j-2);
							MyMesh::EdgeHandle   fe=polymesh.edge_handle(r);
							MyMesh::HalfedgeHandle  he=polymesh.halfedge_handle(fe,0);
					...
					{
						for (j=ymin_index+1;j<=ymax_index+1;j++)
						{
							r=3*m1+1+(2*m1+1)*(j-2);
							MyMesh::EdgeHandle   fe=polymesh.edge_handle(r);
							MyMesh::HalfedgeHandle  he=polymesh.halfedge_handle(fe,0);
			...
			//再遍历各行的边
			for(i=ymin_index;i<=ymax_index+1;i++)
			{
				if (i==1)
				{
					if (xmin_index==0)
					...
					{
						for (j=xmin_index+1;j<=xmax_index+1;j++)
						{
							r=3*j;
							MyMesh::EdgeHandle   fe=polymesh.edge_handle(r);
							MyMesh::HalfedgeHandle  he=polymesh.halfedge_handle(fe,0);
			...
			MyMesh::EdgeIter  ei,e_end(polymesh.edges_end());
			for (ei=polymesh.edges_begin();ei!=e_end;++ei)
			{
				if (!polymesh.is_boundary(*ei))
				{
                 polymesh.property(e_area,*ei)=(polymesh.property(edge_curvature,*ei))/(polymesh.property(num_curvature,*ei))*(polymesh.calc_edge_length(*ei));
				}
			}
...
//求解目标值
	...
	for (face_iter=polymesh.faces_begin();face_iter!=face_end;++face_iter)
	{
      g+=pow(polymesh.property(f_area,*face_iter)-curaverage,2);
	}
    *func=g;
   ...
   vector<double> uknot_grad(unum-2),vknot_grad(vnum-2);//分别存储每条节点线的梯度
	...
	{
      grad[i]=uknot_grad[i];
	  gradnorm+=pow(grad[i],2);
	}
for (int j=unum-2;j<unum+vnum-4;j++)
{
	grad[j]=vknot_grad[j-unum+2];
	 gradnorm+=pow(grad[j],2);
}
