...
void MeshViewer::draw()
{
	...
	if(borigina_mesh_view || bfitted_mesh_view)
	{
...
void MeshViewer::draw_fitted_mesh()
{
	...
	if(mesh != NULL && mesh->is_valid() && !mesh->empty())
	{
		Polyhedron::Face_iterator fit = mesh->facets_begin();
...
//}
//
void MeshViewer::update_view(CSurfaceData *data)
{
	surfacedata = data;
