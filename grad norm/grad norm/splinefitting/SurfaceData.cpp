...
void CSurfaceData::color_polymesh()
{
			...
			if (!polymesh.has_face_colors())
				...
				{
					double ggg=polymesh.property(f_area,*face_iter);
		...
		/*************************************/
			...
			for (face_iter=polymesh.faces_begin();face_iter!=face_end;++face_iter)
			{
				double  f=polymesh.property(f_area,*face_iter);
				g3=abs(polymesh.property(f_area,*face_iter)-curaverage)/curaverage;
				error_cur.push_back(g3);
				polymesh.property(f_error,*face_iter)=g3;
			}
