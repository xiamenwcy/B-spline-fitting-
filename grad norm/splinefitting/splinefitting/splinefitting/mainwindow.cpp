...
void mainwindow::createActions()
{
   ...
   // connect(saveCEAction, SIGNAL(triggered()), this, SLOT(saveAs()));
	...
	KnotIterationAction=new QAction(tr("KnotIteration times  Input"),this);
	KnotIterationAction->setCheckable(true);
	connect(KnotIterationAction, SIGNAL(triggered()), this, SLOT(knots_iteration()));
...
void  mainwindow::knots_iteration()
{
	bool ok;
    int value1=QInputDialog::getInt(this,tr("Iteration Times inputting"),tr("Please input iteration times :"),10,1,100,1,&ok);
	if(!ok)
		return;
	...
	if(!surfacedata->knots_iteration())
   {
