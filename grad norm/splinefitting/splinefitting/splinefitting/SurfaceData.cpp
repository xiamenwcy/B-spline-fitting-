...
typedef  vector<vector< OpenMesh::Vec3f> > vectex;
using namespace std;
...
bool   CSurfaceData::knots_iteration()
{
	...
	//求解平均曲率积分总和
     float sum=0.0f;
	  ...
	  {
		  float ts2=m_pOriginalMesh->property(f2_mean_curvature,*f_it);
		  TexCoord A,B,C;
...
void CSurfaceData::color_polymesh()
{
	...
	//ofstream  test_error("error.txt");
	OpenMesh::FPropHandleT<float> f_area;  //增加每个面的曲率面积属性
	polymesh.add_property(f_area,"f_area");
	polymesh.property(f_area).set_persistent(true);
	//OpenMesh::FPropHandleT<float>  fr_area;//增加每个面的真实面积
	//polymesh.add_property(fr_area,"fr_area");
	OpenMesh::FPropHandleT<vectex> f_point;//增加每个面的凸多边形的坐标
	polymesh.add_property(f_point,"f_point");
	polymesh.property(f_point).set_persistent(true);
	...
	for (f_it=m_pOriginalMesh->faces_begin();f_it!=f_end;++f_it)
	{
		...
		intersects.erase( std::unique( intersects.begin(), intersects.end(),check_func), intersects.end() );
		...
		for(iter=intersects.begin();iter!=intersects.end();++iter)
			for (int j=0;j<n;j++)
			{
				...
				else if (abs((*iter)[0]-uknots[xmin_index+j+1])<=EPSINON&&(j+1)<n)
				{
					vertical_pointsets[j].push_back(*iter);
			...
			for(int j=0;j<n;j++)
			{
				...
				for (int k2=ymin_index+1;k2<=ymax_index;k2++)
				{
					if (intersection_v(vertical_pointsets[j][mj-1],vertical_pointsets[j][0],vknots[k2],&test2)==1)
					{
						intersects_v.push_back(test2);
					}
				}
				...
				intersects_v.erase( std::unique( intersects_v.begin(), intersects_v.end(),check_func ), intersects_v.end() );
				//分配交点到各个水平条形中 
				for(iter_v=intersects_v.begin();iter_v!=intersects_v.end();++iter_v)
					for (int t=0;t<m;t++)
					{
						...
						else if (abs((*iter_v)[1]-vknots[ymin_index+t+1])<=EPSINON&&(t+1)<m)
						{
							horizon_pointsets[t].push_back(*iter_v);
					...
					for (int k1=0;k1<m;k1++)//交换各个条形中的顶点（共3-7个交点）使之成为逆时针方向
					{
						reorder_knots(&(horizon_pointsets[k1]));
						assert(area(horizon_pointsets[k1])>=0.0f);
						int y_index=ymin_index+k1;
						int  index=y_index*(unum-1)+x_index;
						f2_it=polymesh.faces_begin();
						advance(f2_it,index);
						float t11=area(horizon_pointsets[k1]);
						float t22=(float)(m_pOriginalMesh->property(f_mean_curvature2,*f_it));
						vector<OpenMesh::Vec3f>  horizon_pointsets_plus;//水平条形中所含顶点集
						vector<TexCoord>::iterator pp;
						for (pp=horizon_pointsets[k1].begin();pp!=horizon_pointsets[k1].end();++pp)
						{
							horizon_pointsets_plus.push_back(OpenMesh::Vec3f(t22,(*pp)[0],(*pp)[1]));
						}
						polymesh.property(f_point,*f2_it).push_back(horizon_pointsets_plus);//第一个为曲率值，第二、三个坐标为Texcoord
						polymesh.property(f_area,*f2_it)+=t11*t22;
	...
	for (face_iter=polymesh.faces_begin();face_iter!=face_end;++face_iter)
	{
		...
		op+=powf(polymesh.property(f_area,*face_iter)-curaverage,2);
		
	}
	error_sum.push_back(op);

std::cout<<"第"<<" "<<iter_num++<<" "<<"次迭代,函数值:"<<op<<" ";
	max_curerror=*(max_element(error_cur.begin(),error_cur.end()));
	min_curerror=*(min_element(error_cur.begin(),error_cur.end()));
	//准备梯度
	...
	{
		float fcsum=0.0f;//节点线左右面的曲率积分之和
		for (pp=(*oo).begin();pp!=(*oo).end();++pp)
		{
			...
			ef1=polymesh.face_handle(inter_index[0]);
			ef2=polymesh.face_handle(inter_index[1]);
			float tsts=(polymesh.property(f_area,ef1)-polymesh.property(f_area,ef2))*polymesh.property(e_area,ee);
			uknot_grad[k_iter]+=tsts;
			fcsum+=(polymesh.property(f_area,ef1)+polymesh.property(f_area,ef2));
		}
		uknot_grad[k_iter++]*=2;
	}
	double gradnorm=0.0;
	...
	{
		float fcsum=0.0f;//节点线上下面的曲率积分之和
		for (pp=(*oo).begin();pp!=(*oo).end();++pp)
		{
			inter_index.clear();
			ee=polymesh.edge_handle(*pp);
	...
	for (face_iter=polymesh.faces_begin();face_iter!=face_end;++face_iter)
	{
		...
		QColor c;
		double ratio = error/max_curerror;
		c.setHsv( h2 + qRound( ratio * ( h1 - h2 ) ),
			s2 + qRound( ratio * ( s1 - s2 ) ),
...
float uh=udmin/10*pow(0.5,k/static_cast<double>(iter_times-k));
float vh=vdmin/10*pow(0.5,k/static_cast<double>(iter_times-k));
for (int i=1;i<=unum-2;i++)
{
	float  dis2=uh*uknot_grad[i-1]/abs(uknot_grad[i-1]);
	uknots[i]-=dis2;
}
for (int j=1;j<=vnum-2;j++)
{
	float  dis2=vh*vknot_grad[j-1]/abs(vknot_grad[j-1]);
	vknots[j]-=dis2;
...
//	return false;
//}
int CSurfaceData::intersection_uhull_check(float u,vector<TexCoord> polygon,vector<TexCoord> *intersection,bool *right )
{
	vector<float>  yit;
    vector<TexCoord>::iterator it;
	TexCoord intersection2(0.0f,0.0f);
	int num=polygon.size();
	for (it=polygon.begin();it!=polygon.end();++it)
	{
		yit.push_back((*it)[0]);
	}
	float xmin=*(min_element(yit.begin(),yit.end()));
	float xmax=*(max_element(yit.begin(),yit.end()));
if (xmax<=u)
{
*right=false;
return 0;
...
else
{
	...
	{
		int internum=intersection_u(polygon[i],polygon[i+1],u,&intersection2);
		if (internum==-1||internum==1)
		{
			intersection->push_back(intersection2);
		}
	}
	int internum=intersection_u(polygon[0],polygon[num-1],u,&intersection2);
	if (internum==-1||internum==1)
	{
		intersection->push_back(intersection2);
	}
   //去重复性  
	sort(intersection->begin(),intersection->end());//字典序排序
	intersection->erase( std::unique( intersection->begin(), intersection->end(),check_func ), intersection->end() );
    return intersection->size();
}


}
int CSurfaceData::intersection_vhull_check(float v,vector<TexCoord> polygon,vector<TexCoord> *intersection,bool *upper )
{
	vector<float>  yit;
	vector<TexCoord>::iterator it;
	TexCoord intersection2(0.0f,0.0f);
	...
	else
	{
		for (int i=0;i<num-1;i++)
		{
			int internum=intersection_v(polygon[i],polygon[i+1],v,&intersection2);
			if (internum==-1||internum==1)
			{
				intersection->push_back(intersection2);
			}
		}
		int internum=intersection_v(polygon[0],polygon[num-1],v,&intersection2);
		if (internum==-1||internum==1)
		{
			intersection->push_back(intersection2);
		}
...
bool CSurfaceData::check_func(TexCoord i, TexCoord j)
{
	if (abs(i[0]-j[0])<=EPSILON2 && abs(i[1]-j[1])<=EPSILON2)
		return true;
	else
		return false;
}
...
else if (A[0]<B[0])
{
	...
	else if (abs(u-A[0])<=EPSINON||abs(u-B[0])<=EPSINON)
	{
		v=(B[1]-A[1])/(B[0]-A[0])*(u-B[0])+B[1];
		intersection->data()[0]=u;
		intersection->data()[1]=v;
