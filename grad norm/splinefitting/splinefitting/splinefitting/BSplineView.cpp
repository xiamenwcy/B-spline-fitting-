...
bool CBSplineSurfaceView::solvecontrolpoint(Mesh *mesh)
{
	...
	float *uknot1=parameter->getuknot();//uknot1是从up到u_m+1
	float *vknot1=parameter->getvknot();
	float *uknot=new float[m+p+2];//uknot是从u0到u_m+p+1
...
//k代表阶数
float CBSplineSurfaceView::Base(int i,int k,float knot[],int num,float u)//计算B样条基函数
{
	float  Val=0.0;
	float  val1=0.;
	float  val2=0.;
